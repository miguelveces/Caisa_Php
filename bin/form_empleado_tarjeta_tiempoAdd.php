...
else
{
	...
	//{
		...
		for ($x = 0; $x <= $date->format('%d'); $x++) 
		{
			...
			$param = array('fecha_dia_feriado' => $fecha); 
			$resultFeriado = $client->call('GetDayHolidayByfecha',$param,'','','',true);
			$dia=$date1->format('l');
			$laboro="1";
			$ausencia="";
			$tipo="R";
			if($dia=="Sunday"){$tipo="D";$laboro="0";$ausencia="L";}
			if($resultFeriado[0]['id_dia_feriado']!=null){$tipo="F";$laboro="0";$ausencia="L";}
			$codigo="N";
			$codigo_jornada="01";
			$com="";
			$hora_entra="07:00 AM";
 ...
 $json = array("status" => 1,"info" => $result,"period" =>$resultPeriod,"workingday" =>$resultWorkingDayEmployee);
